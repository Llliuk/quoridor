...
bool border_boundaries1(char **m, player *plr1, player *plr2, int move){
    ...
    if(move==11||move==22||move==33||move==44||move==12||move==13||move==21||move==24||move==42||move==43){
        if(face_to_face(plr1,plr2,move)==true){ /* here i'll must write control for the barriers check return function */
            switch(move){
                case 11:
                    if(plr1->x-4<0){
                        return false;
        ...
        if(face_to_face(plr1,plr2,move)==false){
            switch(move){
                case 1:
                    if(plr1->x-2<0){
                        return false;
...
It works for both player controls */

bool face_to_face(player *plr1, player *plr2, int move){
